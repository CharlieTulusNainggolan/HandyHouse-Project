...
app.post('/api/hr/upload', upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ status: 'error', message: 'No file uploaded' });
    }

    const bucketName = 'hr-assets';
    const objectName = `hr/${Date.now()}-${req.file.originalname.replace(/[^a-zA-Z0-9.-]/g, '_')}`;
...
app.get('/api/hr/health', async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 'ok', service: 'HR Service', dbTime: result.rows[0].now });
  } catch (error) {
...
app.get('/api/hr/employees', (req, res) => {
  res.status(200).json([{ id: 1, name: 'Employee X' }, { id: 2, name: 'Employee Y' }]);
});
